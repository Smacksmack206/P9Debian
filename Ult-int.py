...
# THIS SECTION MUST EXIST BEFORE THE FIRST @installer_step DECORATOR IS USED
installer_steps = [] # List to hold step info

# Decorator function to register installer steps
def installer_step(title):
    ...
    def decorator(func):
        ...
        # Store the title and the function itself in the list
...
# Now the @installer_step decorators can be used
...
def step_install_deps(progress, task_id):
    ...
    if not install_result:
        ...
        if not fix_result:
             ...
             return False # Cannot proceed if dependencies cannot be fixed

        # Retry installation after attempting fix
        console.print("Retrying package installation after fix attempt...")
        with console.status("[bold cyan]Running apt-get install (retry)...", spinner="dots"):
        ...
        if not install_result:
             ...
             return False # Cannot proceed without required packages
        else:
    ...
    # --- Use shutil.which for verification ---
    ...
    for cmd in KEY_COMMANDS_TO_VALIDATE:
        ...
        cmd_path = shutil.which(cmd) # Check using shutil.which()
        if cmd_path is None:
            ...
            all_found = False
             ...
             # Log success and path found
             logger.info(f"Verification success: Command '{cmd}' found at {cmd_path}")
             console.log(f"[green]✓[/green] Command '{cmd}' found at: [dim]{cmd_path}[/dim]") # Use log to be less verbose on console
...
def step_create_qcow(progress, task_id):
    ...
    # File does not exist, confirm creation with user
    ...
    # User confirmed, proceed with creation using qemu-img
    ...
    # Use status spinner for potentially long operation
    with console.status(f"[bold cyan]Running qemu-img create for {DEFAULT_QCOW_SIZE} file...", spinner="earth"):
        create_result = run_command(create_cmd, description=f"Creating {DEFAULT_QCOW_SIZE} QCOW2 file", show_output=True) # Show output for errors
    ...
    # Set initial permissions (root:disk 660) immediately after creation
    console.print(f"Setting initial permissions (660, root:disk) for [cyan]{LOCAL_QCOW_PATH}[/cyan]...")
    try:
        # Ensure the 'disk' group exists (should have been created/checked in prereqs)
        if not check_group_exists("disk"):
             # This shouldn't happen if prereqs passed, but handle defensively
        ...
        os.chmod(LOCAL_QCOW_PATH, 0o660) # Set permissions to rw-rw----
        os.chown(LOCAL_QCOW_PATH, 0, disk_gid) # Set owner to root (UID 0) and group to 'disk'
        ...
        # Permissions are critical for NBD/LVM steps, abort if they fail
        return False
...
def step_set_timezone(progress, task_id):
    ...
    # Use a timezone relevant to the user's location (Seattle -> America/Los_Angeles)
    ...
    if run_command(['timedatectl', 'set-timezone', timezone], description=f"Setting timezone to {timezone}"):
        ...
        return True
         ...
         progress.update(task_id, advance=1) # Advance anyway
         return True
...
def step_zerotier(progress, task_id):
    ...
    if not zt_check_result:
        ...
        # Use run_command with shell=True carefully for the pipe
        zt_install_cmd = "curl -s https://install.zerotier.com | bash"
        with console.status("[bold cyan]Running ZeroTier install script...", spinner="moon"):
            install_result = run_command(zt_install_cmd, description="Downloading and running ZeroTier installer", shell=True, show_output=True) # Show output
        if not install_result:
    ...
    # Use enable --now for combined action
    ...
    if not enable_start_result:
        # Check if it's already active, as enable --now might fail if already running
        ...
        if not status_result or status_result.returncode != 0:
             ...
             return False # Fail if service cannot be started/enabled
        else:
    ...
    time.sleep(2) # Give the service a moment to initialize fully
    ...
    if not network_joined:
         ...
         if not join_result:
              ...
              # Continue, as authorization in ZeroTier Central is required anyway
         else:
         ...
         # Always remind user to authorize
    ...
    # Display network and IP info again after potential join attempt
...
def step_ssh_prep(progress, task_id):
    ...
    if not mkdir_result:
         ...
         return False # Cannot proceed if dir cannot be created

    # Set permissions as the target user
    ...
    if not chmod_dir_result:
         ...
         return False # Permissions are critical

    console.print(f"Ensuring authorized_keys file [cyan]{auth_keys_file}[/cyan] exists with 600 permissions...")
    # Touch the file as the target user to create if missing or update timestamp
    ...
    # Set permissions on the file as the target user
...
# --- CORRECTED step_groups_xorg function ---
...
def step_groups_xorg(progress, task_id):
    ...
    progress.update(task_id, advance=1)
    return True
# --- END CORRECTED step_groups_xorg function ---
...
def step_rust_just(progress, task_id):
    ...
    if not cargo_exists:
        ...
        # Command to download and run rustup installer non-interactively
        rustup_cmd = "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --no-modify-path"
        # Execute the command as the target user using bash -c
        with console.status("[bold cyan]Running rustup installer...", spinner="dots"):
             rustup_install_result = run_command(['bash', '-c', rustup_cmd], user=DEBIAN_USER, description="Running rustup installer", show_output=True) # Show output
        ...
        # Add cargo bin directory to user's PATH in .profile if not already present
        ...
        try:
            # Ensure .profile exists (touch as user)
            if not run_command(['touch', str(profile_path)], user=DEBIAN_USER, description=f"Ensuring {profile_path} exists"):
                 ...
                 # Continue, but appending might fail if file doesn't exist and couldn't be touched

            # Check if the line already exists (grep as user)
            # Use -F for fixed string, -x for whole line match
            ...
            if not path_exists_result or path_exists_result.returncode != 0:
                 # Line doesn't exist, append it using tee as user
                 # Ensure echo output has newline
                 ...
                 if not append_result:
                      ...
                      return False # Fail if PATH cannot be set
    ...
    # Install 'just' command runner using cargo
    console.print("Checking for 'just' command runner...")
    logger.info(f"Checking for 'just' for user {DEBIAN_USER}.")
    # Ensure PATH includes ~/.cargo/bin for the check and potential install
    # Best way is to prepend path within the command executed as the user
    ...
    if not just_exists:
        ...
        with console.status("[bold cyan]Running cargo install just...", spinner="dots"):
            install_just_result = run_command(['bash', '-c', just_install_cmd], user=DEBIAN_USER, description="Installing just via cargo", show_output=True) # Show output
...
def step_zt_join_verify(progress, task_id):
     ...
     # Display current networks and IP addresses
     run_command(['zerotier-cli', 'listnetworks'], description="Current ZeroTier Networks", show_output=True, check=False)
     run_command(['ip', '-brief', 'addr'], description="Current IP Addresses", show_output=True, check=False)
     # Remind user about authorization
...
def step_qcow_perms(progress, task_id):
    ...
    try:
        # Ensure 'disk' group exists (should be guaranteed by prereqs if fatal was set)
        if not check_group_exists("disk"):
        ...
        disk_gid = grp.getgrnam("disk").gr_gid
        target_mode = 0o660 # Target permissions (rw-rw----)
        target_uid = 0 # Target owner (root)
        target_gid = disk_gid # Target group ('disk')

        # Get current file status
        current_stat = LOCAL_QCOW_PATH.stat()
        current_mode = current_stat.st_mode & 0o777 # Extract permission bits
        ...
        # Check and set ownership if needed
...
def step_nbd_service(progress, task_id):
    ...
    # Using Type=simple and RemainAfterExit=yes with --persistent
    content = f"""[Unit]
Description=Connect QEMU NBD device {NBD_DEVICE} to {LOCAL_QCOW_PATH}
Documentation=man:qemu-nbd(8)
# Start after basic system initialization and network (if QCOW2 is remote, though unlikely here)
After=local-fs.target network-online.target systemd-modules-load.service
Wants=network-online.target
# Ensure NBD runs before LVM attempts activation on this device
Before=lvm2-activation-early.service lvm2-activation.service lvm-activate-data-vg.service

[Service]
Type=simple
# Keep service active as long as qemu-nbd process runs (needs --persistent)
RemainAfterExit=yes
Restart=on-failure
RestartSec=5s
# Ensure nbd kernel module is loaded, allow failure if already loaded
ExecStartPre=-/sbin/modprobe nbd nbds_max=16
# Cleanly disconnect any lingering connection on the device before starting
ExecStartPre=-/usr/bin/qemu-nbd --disconnect {NBD_DEVICE}
# Connect NBD device persistently. qemu-nbd stays in foreground with Type=simple.
ExecStart=/usr/bin/qemu-nbd --connect={NBD_DEVICE} --persistent {LOCAL_QCOW_PATH}
# Disconnect the NBD device when the service is stopped
...
# Requires the NBD device service to have successfully started and udev settled
Requires=qemu-nbd-connect.service
After=qemu-nbd-connect.service systemd-udev-settle.service
# Activate LVM before filesystems depending on it are mounted
Before=local-fs.target remote-fs.target

[Service]
Type=oneshot
RemainAfterExit=yes # Keep service state active after successful run
Environment="PATH=/usr/sbin:/usr/bin:/sbin:/bin"
# Give udev time to process the NBD device appearing after qemu-nbd starts
ExecStartPre=/bin/sleep 1
ExecStartPre=/usr/bin/udevadm settle --timeout=30
# Check NBD device exists and is readable
...
# Scan the specific NBD device for LVM Physical Volumes and update cache
ExecStartPre=/usr/sbin/lvm pvscan --cache {NBD_DEVICE}
# Activate the Volume Group (-ay enables all LVs within it)
ExecStart=/usr/sbin/lvm vgchange -ay {VG_NAME}
# Wait for the specific Logical Volume device node to appear after activation
...
# Deactivate the Volume Group when the service is stopped
...
def step_lvm_setup(progress, task_id):
    ...
    # Check if the block device for the LV already exists
    if LV_DEVICE_PATH.is_block_device():
        ...
        # Ensure VG is active (service should handle this on boot, but useful here)
        run_command(['lvm', 'vgchange', '-ay', VG_NAME], description="Ensuring VG is active", check=False)
        progress.update(task_id, advance=1)
        return True

    # LV does not exist, proceed with one-time setup
    ...
    # Reload systemd daemon to ensure it knows about the new NBD/LVM services
    ...
    # Start NBD service temporarily to expose the QCOW2 file as a block device
    ...
    if not start_nbd_result:
         ...
         # Show NBD service logs to help diagnose
    ...
    # Wait briefly for service to stabilize and device to appear
    time.sleep(2)

    # Explicitly check if NBD device exists and is readable after start
    # This check is now also in lvm-activate service, but good to double-check here for setup.
    ...
    if not nbd_ready_result or nbd_ready_result.returncode != 0:
        ...
        run_command(['journalctl', '-u', 'qemu-nbd-connect.service', '-n', '50', '--no-pager'], description="NBD service logs", show_output=True, check=False)
        # Attempt to stop the failed service before exiting
    ...
    # Proceed with LVM command sequence (PV, VG, LV, Format)
    lvm_success = True
    with console.status("[bold cyan]Creating LVM structures (pvcreate, vgcreate, lvcreate, mkfs)...[/cyan]", spinner="line"):
        # Create Physical Volume on the NBD device, force (-f) to overwrite any existing data/signatures
        ...
        # Create Volume Group using the newly created PV
        if lvm_success:
        ...
        # Create Logical Volume using 100% of the free space in the VG
        if lvm_success:
        ...
        # Wait for the LV device node to appear in /dev
        if lvm_success:
            ...
            if not node_appeared:
                ...
                lvm_success = False
                 ...
                 # Settle udev rules after node appears
        ...
        if lvm_success:
            ...
            # Use -F to force formatting even if filesystem detected (shouldn't be, but safe)
            if not run_command(['mkfs.ext4', '-F', str(LV_DEVICE_PATH)], description="Formatting LV with ext4"): lvm_success = False


    # Stop the temporary NBD service regardless of LVM outcome
    console.print("Stopping temporary NBD service...")
    logger.info("Stopping transient NBD service.")
    run_command(['systemctl', 'stop', 'qemu-nbd-connect.service'], description="Stopping NBD service", check=False) # Don't fail script if stop fails
    time.sleep(2) # Give time for disconnect

    # Final check on LVM setup success
    if lvm_success:
        ...
        # Provide info for manual diagnosis
...
def step_fstab(progress, task_id):
    ...
    try:
        ...
        # Get UID/GID for ownership setting
        user_info = pwd.getpwnam(DEBIAN_USER)
        group_info = grp.getgrnam(DEBIAN_GROUP) # Assumes DEBIAN_GROUP exists
    ...
    # Using the device path is simpler than UUID here, given the setup context.
    # Options: defaults, nofail (don't halt boot if mount fails), _netdev (requires network, though NBD is local here, LVM depends on NBD service which waits for network)
    ...
    try:
        # Read existing fstab content
        content = fstab_file.read_text()
        entry_exists = False
        # Check if a similar entry (same device and mount point) already exists
        for line in content.splitlines():
            if line.strip().startswith('#'): continue # Skip comments
            parts = line.split()
            # Check if first two fields match our device and mount point
            if len(parts) >= 2 and parts[0] == str(LV_DEVICE_PATH) and parts[1] == str(LVM_MOUNT_POINT):
                ...
                break
            # Check if mount point is used by a different device (potential conflict)
        ...
        if not entry_exists:
            ...
            # Write the modified content back (consider backup first?)
        ...
        # Attempt to mount the filesystem using the fstab entry ('mount -a' might mount others, use specific mount point)
        ...
        # Ensure LVM Volume Group is active first (critical before mount)
        ...
        if not vg_activate_result or vg_activate_result.returncode != 0:
             # This might happen if the LV was just created and VG isn't active yet
        ...
        if not mount_result:
             ...
             # Provide diagnostics
             run_command(['mountpoint', '-q', str(LVM_MOUNT_POINT)], check=False, description="Checking mount status code") # 0 if mounted, non-zero if not
...
def step_enable_storage_services(progress, task_id):
    ...
    if not run_command(['systemctl', 'daemon-reload'], description="Daemon reload"):
        ...
        # This is often non-fatal but indicates a potential issue
    ...
    if nbd_enabled_ok and lvm_enabled_ok:
        ...
        return False # Fail if services cannot be enabled
...
def step_install_brave(progress, task_id):
    ...
    # Check if already installed using shutil.which
    ...
    # Use status context for the multi-step operation (repo setup, install)
    with console.status("[bold cyan]Configuring Brave repository and installing package...", spinner="dots"):
        ...
        # Define the repository line using the detected architecture
        ...
        # Download GPG key using curl (safer than wget | gpg)
        if success:
        ...
        if success:
            # Pass show_content=True to log/display the repo line being written
        ...
        # Update apt package lists after adding the new repository
        if success:
        ...
        # Install the brave-browser package non-interactively
        if success:
            install_env = os.environ.copy()
            install_env['DEBIAN_FRONTEND'] = 'noninteractive'
    ...
    if success:
        ...
        # Installation failed, attempt cleanup of added files
        ...
        # Use unlink(missing_ok=True) for safe removal
...
def step_setup_vnc(progress, task_id):
    ...
    # Define the content for the xstartup script
    xstartup_content = f"""#!/bin/sh

# Ensure script is executable
# chmod +x $HOME/.vnc/xstartup

# Unset potentially problematic environment variables from the VNC service environment
unset SESSION_MANAGER
unset DBUS_SESSION_BUS_ADDRESS

# Set session type and desktop environment for GNOME
export XDG_SESSION_TYPE=x11
export XDG_CURRENT_DESKTOP=GNOME
export GNOME_SHELL_SESSION_MODE=debian
# May need others depending on GNOME version / setup, e.g.:
# export XDG_MENU_PREFIX=gnome-
# export XDG_DATA_DIRS=/usr/share/gnome:/usr/local/share/:/usr/share/

# Source system-wide VNC startup script if it exists (optional)
# [ -x /etc/vnc/xstartup ] && exec /etc/vnc/xstartup

# Load X resources for the session (e.g., .Xresources)
[ -r $HOME/.Xresources ] && xrdb $HOME/.Xresources

# Start the GNOME Desktop Environment
# Using dbus-launch helps ensure D-Bus session is correctly started
# exec replaces the shell, ensuring clean exit when session ends
# Use setsid to ensure it runs in its own session, detaching from VNC server process
# exec setsid dbus-launch --exit-with-session gnome-session
# Simpler approach often works too:
gnome-session &

"""
    # Write the xstartup file, ensuring correct owner/group/permissions
    # The write_file helper handles parent directory creation
    ...
    # Define the systemd service file for VNC
    ...
    # Get user and group info for the service definition
    try:
    ...
    # Content of the systemd service file (vncserver@.service)
    # %i in the filename and commands will be replaced by the display number (e.g., '1')
...
# Start after networking and graphical target dependencies (like display manager)
# Also wait for LVM volume to be active if user's home is on it (though unlikely here)
After=syslog.target network-online.target graphical.target lvm-activate-data-vg.service
Wants=network-online.target

[Service]
Type=forking
# Run commands as the specified user and group
User={DEBIAN_USER}
Group={DEBIAN_GROUP}
# Set home directory for the service process
WorkingDirectory=/home/{DEBIAN_USER}
# Define the command to start the VNC server for the specific display (%i)
# -fg runs the server in the foreground initially, required by Type=forking unless PIDFile is perfect
# -localhost no allows connections from other machines
# -alwaysshared allows multiple clients to connect simultaneously
# -desktop sets the name shown in the VNC client window title
# -SecurityTypes VncAuth,TLSVnc requires vncpasswd setup
...
# Specify the PID file location (matches vncserver default pattern)
PIDFile=/home/{DEBIAN_USER}/.vnc/%H:%i.pid
# Command to stop the VNC server for this display
ExecStop=/usr/bin/vncserver -kill :%i

[Install]
# Enable the service to start on boot within the multi-user graphical environment
WantedBy=multi-user.target
"""
    ...
    if not run_command(['systemctl', 'daemon-reload'], description="Daemon reload"):
        ...
        # Continue, but enablement might fail if daemon didn't reload

    # Enable the specific instance of the service (e.g., vncserver@1.service)
    ...
    if not run_command(['systemctl', 'enable', vnc_instance_service], description=f"Enabling {vnc_instance_service}"):
        ...
        return False # Fail if service cannot be enabled
...
def step_configure_samba(progress, task_id):
    ...
    if smb_conf_file.exists():
         backup_file = smb_conf_file.with_suffix(f".bak-{current_timestamp}") # Use shared timestamp
         try:
              # Use shutil.move for atomic rename (usually)
              ...
              # Ask user if they want to continue? For now, proceed cautiously.

    # Ensure the path to be shared exists and is a directory
    ...
    # Define the content for smb.conf
    # Includes a basic [global] section and the specific share definition
...
[global]
   ...
   # --- Security ---
   # Use user-level security (requires system users or Samba users)
   security = user
   # Map attempts to connect as guest to 'nobody' or a specific guest account if needed
   # 'Bad User' maps unknown users to guest, 'Bad Password' maps failed logins to guest
   map to guest = Bad User
   # Encrypt passwords (required by modern clients)
   ...
   # log level = 1 # Default is usually sufficient
   # debug level = 1

   # --- Networking (Optional) ---
   # interfaces = lo eth0 # Restrict listening interfaces if needed
   # bind interfaces only = yes
   # hosts allow = 192.168.1. 127. # Restrict allowed client IPs if needed

   # --- Domain/Workgroup Interaction (Standalone Server) ---
   server role = standalone server
   # obey pam restrictions = yes # Use PAM for session management if needed
   # unix password sync = yes # Sync Unix password changes to Samba (requires pam_smbpass)
   # pam password change = yes # Allow password changes via PAM

   # --- Performance Tuning (Optional - Defaults often okay) ---
   # socket options = TCP_NODELAY SO_RCVBUF=131072 SO_SNDBUF=131072
   # use sendfile = yes
   # strict locking = no # May improve performance with some apps, but use with caution
   # oplocks = yes
   # level2 oplocks = yes
...
[${SAMBA_SHARE_NAME}]
   ...
   # Share visibility and access
   browseable = yes
   read only = no
   # Allow guest access (no password needed if guest ok = yes and security=user/map to guest=Bad User)
   # Set guest ok = no to require authentication
   guest ok = yes
   # Force files/dirs created via the share to belong to this user/group
   force user = {DEBIAN_USER}
   force group = {DEBIAN_GROUP}
   # Set permissions for new files/dirs created via Samba
   # 0664: rw-rw-r-- for files
   # 0775: rwxrwxr-x for directories
   create mask = 0664
   directory mask = 0775
   # Optional: Make guest access writable? (Requires guest account mapping in [global])
   # guest only = no # If yes, only guest access allowed
   # writeable = yes # Synonym for read only = no
"""
    # Write the smb.conf file
    ...
    # Verify the configuration using testparm
    console.print("[cyan]Verifying Samba configuration using 'testparm'...[/cyan]")
    testparm_result = run_command(['testparm', '-s'], description="Running testparm", show_output=True) # Show output for review
    if not testparm_result:
        # testparm returns non-zero on fatal syntax errors
        ...
        return False
    # Check for specific success indicator in output (optional but good)
    ...
    # Enable and restart Samba services (smbd for file sharing, nmbd for NetBIOS name resolution)
    ...
    # Use enable --now for simplicity, combines enable and start
    enable_start_result = run_command(['systemctl', 'enable', '--now', 'smbd', 'nmbd'], description="Enabling and starting smbd/nmbd")
    if not enable_start_result:
         # If enable --now failed, try restart explicitly (might be needed if already enabled but failed start)
    ...
    # Final status check after enabling/restarting
    ...
    if smbd_active and nmbd_active:
        ...
        run_command(['systemctl', 'status', 'smbd', 'nmbd', '--no-pager'], description="Samba service status", check=False, show_output=True)
        # Decide if this is fatal? Sharing won't work, but maybe let script finish? Let's make it non-fatal warning.
...
def step_setup_podman(progress, task_id):
    ...
    sub_id_count = 65536 # Standard number of subids
    sub_uid_file = Path("/etc/subuid")
    sub_gid_file = Path("/etc/subgid")

    # Configure subordinate UIDs/GIDs for rootless Podman user
    ...
    try:
        ...
        if f"{DEBIAN_USER}:" not in sub_uid_content:
            ...
            # Check if existing entry matches desired range? Overkill for now.
        ...
        logger.info("Subordinate ID configuration finished.")
         ...
         # Catch errors reading/writing subid files
    ...
    # Enable session lingering for the user (allows rootless containers to run after logout)
    ...
    linger_result = run_command(['loginctl', 'enable-linger', DEBIAN_USER], description="Enable linger", check=False) # Fails if already enabled
    if not linger_result or linger_result.returncode != 0:
        # Check the actual linger status
    ...
    # Configure rootful Podman storage path
    ...
    # Recommended driver for most Linux systems
    ...
    # Ensure the target storage directory exists and is owned by root:root
    # Check if mount point is actually mounted first
    ...
    try:
         ...
         os.chown(PODMAN_ROOTFUL_STORAGE_PATH, 0, 0) # Owner: root (UID 0), Group: root (GID 0)
         # Permissions: default mkdir perms (usually 0755) should be okay for rootful storage root
    ...
    # Define the content for /etc/containers/storage.conf
...
[storage]
# Default storage driver for CoW file systems.
driver = "{storage_driver}"

# Default directory to store container images and layers (graph root).
# Set to use the dedicated directory on the LVM volume.
graphroot = "{PODMAN_ROOTFUL_STORAGE_PATH}"

# Default directory to store temporary container state (mount points, etc.).
# Typically placed under /run or /var/run.
runroot = "/run/containers/storage"

# --- Storage Options ---
# Options applied to the storage based on the driver.
[storage.options]
# Size specifies maximum size of created CoW filesystems. (Not typically needed for overlay)
# size = ""

# --- Overlay Driver Specific Options ---
[storage.options.{storage_driver}]
# Ignore chown errors might be needed on file systems like NFS or Lustre.
# ignore_chown_errors = "false"

# Path to the fuse-overlayfs binary. Usually not needed unless multiple versions exist.
# mount_program = "/usr/bin/fuse-overlayfs"

# Options passed to the mount command.
# mount_opt = "nodev,metacopy=on" # metacopy=on can speed up copies on ext4/xfs if supported
...
def step_cleanup(progress, task_id):
    ...
    # Use apt-get clean to remove downloaded package files
    ...
    # Optionally run apt-get autoremove? Can sometimes remove needed packages if deps changed. Be cautious.
    # console.print("[cyan]Removing unused dependency packages (apt-get autoremove)...[/cyan]")
    # if run_command(['apt-get', 'autoremove', '-y'], env=clean_env, description="apt-get autoremove"):
    #    console.print("[green]✓[/green] Unused packages removed.")
    # else:
...
def main():
    ...
    with Progress(
        ...
        for i, step_info in enumerate(installer_steps):
            ...
            # Add task for the current step, initially not started
            step_task = progress.add_task(task_description, total=1, start=False, visible=True)

            # Print rule separator before starting the step
            ...
            progress.start_task(step_task) # Mark task as started for timing

            # Execute the step function, passing progress object and task ID
            step_success = False # Assume failure unless function returns True
            try:
                step_success = step_func(progress, step_task)
            except Exception as step_exception:
                 # Catch unexpected errors within the step function itself
                 ...
                 step_success = False # Ensure marked as failure

            # Update progress based on step outcome
            if step_success:
                # Mark step as completed successfully
                progress.update(step_task, completed=1, description=f"[green]✓ {task_description}[/green]")
                progress.update(overall_task, advance=1) # Advance overall progress
                ...
                progress.stop_task(step_task) # Stop spinner/timer for the failed step
                ...
                # Update overall progress bar to indicate failure
                progress.update(overall_task, description="[bold red]Overall Progress (Failed)[/bold red]")
                progress.stop() # Stop the entire progress display cleanly
                break # Exit the loop immediately on failure

            time.sleep(0.5) # Brief pause for visual flow between steps

        # End of loop - check if all steps succeeded
        if not all_steps_successful:
             # Save console output to an HTML file for easier review on failure
             try:
    ...
    # Final Summary Panel if all steps succeeded
    ...
    # Display Post-Installation Actions and Reminders
    ...
    # Optionally save full console log on success
    # try:
    #     html_log = f"installer_success_console_{current_timestamp}.html"
    #     console.save_html(html_log)
    #     console.print(f"Full console output log saved to [dim]{html_log}[/dim]")
    # except Exception as save_err:
    #     logger.warning(f"Could not save console HTML log on success: {save_err}")
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(130) # Standard exit code for Ctrl+C
         ...
         # Catch any unexpected errors in the main execution flow outside of steps
         console.print(f"\n[bold red]An unexpected critical error occurred outside of step execution:[/bold red]")
         logger.critical("Unexpected critical error during main execution.", exc_info=True) # Log full traceback
         # Use Rich's traceback printing for better console output
         console.print_exception(show_locals=False, word_wrap=True)
